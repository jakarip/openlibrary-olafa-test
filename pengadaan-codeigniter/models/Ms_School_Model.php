<?php
class Ms_School_Model extends CI_Model 
{
	
	private $table = 'ms_school';
	...
	function dtquery($param)
	{
		...
		return $this->db->query("SELECT SQL_CALC_FOUND_ROWS * FROM ".$this->table."
								 LEFT JOIN ms_kec ON school_id_kec = kec_id 
	...
	function getbyid($id)
	{
		return $this->db->query("SELECT * FROM ".$this->table." LEFT JOIN ms_kec ON school_id_kec = kec_id  where school_id='$id'");
	}
