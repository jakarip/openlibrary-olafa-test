...
class ApiModel extends CI_Model {
	...
	{  
		...
		return $this->db->query("select * from member where master_data_user='$user'"); 
	} 
	
	function getMasterDataNumber($number)
	{   
		return $this->db->query("select max(master_data_number)max from member where member_type_id in (19,20,21,22,23) and master_data_number like '".date('ymd').$number."%'");
	} 
	
	function getMasterDataNumberPublic()
	{   
		return $this->db->query("select max(master_data_number)max from member where member_type_id in ('19') and master_data_number like '".date('ymd')."19%'"); 
	} 
