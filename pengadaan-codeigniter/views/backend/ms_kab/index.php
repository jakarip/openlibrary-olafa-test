    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="frm" class="form-horizontal">
                ...
                <input type="hidden" name="kab_id_prov_ori" id="kab_id_prov_ori">
                <div class="modal-body">
                    <div class="box-body" style="padding-bottom:0px">
                        <div class="form-group form-group-sm">
                            <label for="pus_name" class="col-sm-3 control-label">Provinsi</label>
                            <div class="col-sm-9">
                            	<?= form_dropdown('inp[kab_id_prov]', $prov, '', 'class="form-control input-sm select2" id="kab_id_prov" required="true"') ?>
                            </div>
                        </div>
                        <div class="form-group form-group-sm">
                            <label for="pus_name" class="col-sm-3 control-label">Nama Kabupaten</label>
                            <div class="col-sm-9">
                            	<input type="text" class="form-control input-sm" name="inp[kab_name]" id="kab_name" required>
                            </div>
                        </div>
                    </div><!-- /.box-body -->
                </div>
...
var tb 		= '#table';      
var baseurl = '<?= y_url_admin() ?>/ms_kab';
...
{		
	$.ajax({
		...
		success: function(e) {
			...
			$('#kab_id_prov').val(e.kab_id_prov).trigger('change');
			$('#kab_id_prov_ori').val(e.kab_id_prov);
			$('#frmbox').modal({keyboard: false, backdrop: 'static'});
		},
		error : function() {
...
function _reset()
{
	...
	$('#kab_id_prov').val('').trigger('change');
