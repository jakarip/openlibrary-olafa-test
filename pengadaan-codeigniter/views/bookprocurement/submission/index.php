<div class="panel panel-default flat">
			...
			<div class="form-group"> 
				<select name="faculty" id="faculty" class="select2"> 
					...
					<?php foreach($faculty as $row){ ?>
						<option value="<?= $row->c_kode_fakultas ?>"><?= $row->nama_fakultas ?></option>
					<?php } ?>
				</select> 
			</div>
		</div> 
				...
				<select name="prodi" id="prodi" class="select2"> 
						...
						if($facid!=""){
							foreach($prodi as $row){ ?>
								<option value="<?= $row->C_KODE_PRODI ?>" ><?= $row->NAMA_PRODI ?></option>
							<?php }
						} ?>
				</select> 
			</div>
	...
	<div class="row" style="margin:20px 15px;">	
		...
		<div class="col-sm-2" >
			<input type="text" class="form-control input-sm" name="dates_submission" id="dates_submission" value="<?=($status2!="")?$date1.' - '.$date2:''?>">
		</div> 
		<div class="col-md-2">
			<div class="form-group">
 ...
 <div class="modal fade" id="frmbox5" role="dialog" aria-hidden="true">
	 <div class="modal-dialog">
		 <div class="modal-content">
			 <div class="modal-header bg-danger-700" style="background-image:url(assets/images/backgrounds/bg.png)">
				 <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				 <h4 class="modal-title"><i class="fa fa-navicon"></i> &nbsp;<?= 'Form Data Konfirmasi Email' ?></h4>
			 </div>
 ...
 <div class="modal fade" id="frmbox6" role="dialog" aria-hidden="true">
	 <div class="modal-dialog">
		 <div class="modal-content">
			 <div class="modal-header bg-danger-700" style="background-image:url(assets/images/backgrounds/bg.png)">
				 <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				 <h4 class="modal-title"><i class="fa fa-navicon"></i> &nbsp;<?= 'Form Data Ketersediaan Buku' ?></h4>
			 </div>
...
var tb 		= '#table';      
var baseurl = 'index.php/<?= y_url_apps('bookprocurement_url') ?>/submission';
...
$("#status,#prodi,#type,#book_type,#faculty").select2({
...
$("#dates_submission_option").change(function(){ 
 ...
 $('#faculty').change(function() {
	...
	if(facultyId) {
		$.ajax({
			'url':'index.php/<?= y_url_apps('bookprocurement_url') ?>/dashboard/getProdi', 
			type: 'POST',
			...
			success: function(data) {
				...
				<?php			
					if($prodiid!=""){ 	
				?>
   				$("#prodi").val('<?=$prodiid?>').trigger('change');
				<?php				
					}
					if($facid!=""){ 	
				?>
...
?>
   $("#dates_submission_option").val('date').trigger('change');
   $("#faculty").val('<?=$facid?>').trigger('change'); 
   $("#status").val('<?=$status2?>').trigger('change');
<?php
}
else { 
	if($status2!=""){ 	
