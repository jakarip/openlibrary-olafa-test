...
class Submission extends CI_Controller 
{
	function __construct()
	{
		...
		$this->load->model('bookprocurement/Submission_Model', 'sm', TRUE); 
	...
	public function index($filter="",$faculty="",$prodi="",$status2="")
	{ 
		$data['view'] 	= 'bookprocurement/submission/index';
	...
	public function json()
	{
		...
		foreach($result as $row)
		{
			...
			if($row->book_status == 'r_ketersediaan')
			{
					...
					$available = ' <a href="javascript:edit_available('.$row->book_id.')" title="Input Tanggal Ketersediaan Buku" class="btn btn-xs btn-icon btn-primary"><i class="icon-calendar"></i></a>'; 
					$email_confirmed = ' <a href="javascript:edit_email_confirmed('.$row->book_id.')" title="Input Tanggal Konfirmasi Email" class="btn btn-xs btn-icon btn-success"><i class="icon-calendar"></i></a>'; 
			...
			else if($row->book_status == 's_email')
			{
			...
			$rows = array (
				...
				// $jenis_anggota[$row->master_data_type].'<br>'.$row->master_data_institution, 
					$checkbox,
					$btn.$logistic.$accept.$available.$email_confirmed,
					$status,
					$row->NAMA_PRODI, 
					$row->book_member, 
					$row->book_subject, 
		...
		foreach($rowdata as $row)
		{
			...
			else
			{
				...
				$item[] = array('book_id_prodi' => $id[0],
								...
								'book_status' => $status,
								'book_date_available' => $row[15],
								'book_date_email_confirmed' => $row[16],
	...
	public function save_email_confirmed()
	{
		...
		$dt = $this->sm->get_email_confirmed_and_available($id)->row(); 
		if($dt->book_status=='r_ketersediaan') $item['book_status'] = 's_email';
	...
	public function save_available()
	{
		...
		$dt = $this->sm->get_email_confirmed_and_available($id)->row(); 
		if($dt->book_status=='penerimaan') $item['book_status'] = 'r_ketersediaan';
