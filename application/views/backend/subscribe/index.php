		...
		<div class="col-md-3"> 
			<div class="form-group">
				<select name="subs" id="subs" class="select"> 
						...
						<option value="0">Belum Validasi Pembayaran</option>
						<option value="2">Menunggu Verifikasi Admin</option>
		...
		<div class="col-md-2">
			<div class="form-group">
				<select name="dates_option" id="dates_option" class="select"> 
						<option value="all">Semua Tanggal Pembayaran</option>
						<option value="date">Range Tanggal Pembayaran</option>
				</select> 
			</div>
		</div> 
		<div class="col-sm-3" >
...
var tb 		= '#table';      
var baseurl = 'index.php/<?= y_url_admin() ?>/subscribe';
...
function reject_form(sub_id,id)
{
	$.ajax({
...
function approval(sub_id,no,id,usergroup)
{ 
    if(confirm('No Transaksi : '+no+'.\nApakah anda yakin akan melakukan approval ?')) {
        $.ajax({
            ...
            dataType:'json',
            type:'post',
            data: ({ id : id,sub_id : sub_id,usergroup : usergroup }),
            success: function(e) { 
                _reload();
            },
            error : function() {
