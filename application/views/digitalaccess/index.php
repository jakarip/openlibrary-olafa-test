	...
	<div class="panel">
		<div class="panel-header bg-red">
			...
			<div class="row">
				...
				<div class="text-right col-lg-1" style="margin-top:1px;padding-top:7px;">
					<?php echo getLang('Pilih Tahun Access') ?> </div>
			...
			<div class="row">
				...
				<div class="text-right col-lg-2" style="margin-top:1px;padding-top:7px;">
					<?php echo getLang('Akses Via') ?> </div>
				<div class="text-right col-lg-1" style="margin-top:1px;">
			...
			<table class="dt_table1 table table-striped table-bordered table-hover">
				<thead>
					<tr>
						...
						<th width="15%"><?php echo getLang("jenis katalog") ?></th>
						<th width="15%"><?php echo getLang("judul") ?></th>
						<th width="15%"><?php echo getLang("pengarang") ?></th>
						<th width="15%"><?php echo getLang("nama File") ?></th>
						<th width="15%"><?php echo getLang("Tgl Akses") ?></th>
						<th width="15%"><?php echo getLang("Access Via") ?></th>
					</tr>
				</thead>
				<tbody>
				</tbody>
...
function dt_table(grow_year,val,faculty,access) {
	table = $('.dt_table1').DataTable({ 
        ...
        "ajax": {
            "url": 'index.php/digitalaccess/ajax_index',
            "type": "POST",
			"data" : {
				grow_year : grow_year,
				type : val,
...
function excel(id,year,grow_year) {
	$.ajax({
        url : 'index.php/digitalaccess/excel',
        type: "POST",
		data: {
				year :year,
				grow_year:grow_year,
...
function excel_header() {
	$.ajax({
        url : 'index.php/digitalaccess/excel_header',
        type: "POST",
		data: {
				year : $("#tahun").val(),
...
function totalcollection(grow_year,year,faculty) {
	$.ajax({
        url : 'index.php/digitalaccess/totalcollection',
        type: "POST",
		data: {
				year : year,
				grow_year : grow_year,
