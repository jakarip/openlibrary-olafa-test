<?php
class Ms_Location_Model extends CI_Model 
{
	
	private $table = 'ms_location';
	...
	function dtquery($param)
	{
		...
		return $this->db->query("SELECT SQL_CALC_FOUND_ROWS * FROM ".$this->table."
								 LEFT JOIN ms_kec ON location_id_kec = kec_id 
	...
	function getbyid($id)
	{
		return $this->db->query("SELECT * FROM ".$this->table." LEFT JOIN ms_kec ON location_id_kec = kec_id  where location_id='$id'");
	}
	...
	**/
    ...
    {
        return $this->db->query("SELECT * FROM ms_location WHERE location_status = '1'");
    }
}
?>