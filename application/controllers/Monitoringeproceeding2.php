...
class Monitoringeproceeding2 extends CI_Controller {
    ...
    function index($choose="",$type="") { 
		...
		if (!empty($choose) && !empty($type)) {
			...
			$data['jurusan'] = $this->MonitoringEproceedingModel->getProdiByEprocList($list->list_kode_fakultas)->result();
	...
	function generate() { 
		...
		if ($submit!="" and $choose!="0" and $list!="0"){
			...
			foreach($jurusan as $jur) {	
				...
				$html .='
						...
						<tr>
							<td>&nbsp;&nbsp;<strong><input class="selectAllPDF" type="checkbox" name="" value="'.$jur->C_KODE_PRODI.'">&nbsp;&nbsp;Check All PDF</strong>&nbsp;&nbsp;&nbsp;&nbsp;<input data-id="'.$jur->C_KODE_PRODI.'"" type="button" value="Download PDF" class="btn btn-success btn-embossed downloadPDF">&nbsp;&nbsp;<strong><input class="selectAllWord" type="checkbox" name="" value="'.$jur->C_KODE_PRODI.'">&nbsp;&nbsp;Check All Word</strong>&nbsp;&nbsp;&nbsp;&nbsp;<input data-id="'.$jur->C_KODE_PRODI.'"" type="button" value="Download Word" class="btn btn-success btn-embossed downloadWord"></td>
						</tr>
						<tr>
						<td><ol>'; 
				foreach ($dt as $row){
					...
					if (file_exists($eproc)){ 
						...
						$data['jurnal_eproc']++;
						$catalog[] = $row->code.'_jurnal_eproc.pdf'; 
						...
						$html.='<li>&nbsp;&nbsp;<input class="checkPDF'.$jur->C_KODE_PRODI.'" type="checkbox"  name="downloadPDF'.$jur->C_KODE_PRODI.'" value="'.$pdf.'"> <a '.(!in_array($row->code,$art)?'style="color:red"':'').' target="_blank" href="'.$link_download_pdf.'">PDF</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input class="checkWord'.$jur->C_KODE_PRODI.'" type="checkbox"  name="downloadWord'.$jur->C_KODE_PRODI.'" value="'.$word.'">&nbsp;&nbsp;<a '.(!in_array($row->code,$art)?'style="color:red"':'').' target="_blank" href="'.$link_download_word.'">Word</a>&nbsp;&nbsp;'.ucwords(strtolower($row->title));

						$html.='<br>'.$row->master_data_number.' - '.ucwords(strtolower($row->master_data_fullname)).(!empty($row->editor)? ', '.ucwords(strtolower($row->editor)):'').'</li>';
		...
		$data['menu'] 	= 'monitoringeproceeding/monitoringeproceeding_generate2'; 
		$this->load->view('theme', $data);
    }
