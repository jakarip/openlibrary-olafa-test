...
class Ms_Admin extends CI_Controller 
{
	function __construct()
	{
		parent::__construct();
		
		$this->load->model('Admin_Model', 'dm', TRUE);
	...
	{
		$data['view'] 	= 'backend/ms_admin/index';
	...
	public function json()
	{
		...
		foreach($result as $row)
		{
			$rows = array (
				$row->admin_name,
				$row->admin_username,
				'<a href="javascript:edit('.$row->admin_id.')" title="Edit Data" class="btn btn-xs btn-icon btn-primary"><i class="icon-database-edit2"></i></a>
				<a href="javascript:del('.$row->admin_id.', \''.$row->admin_name.'\')" title="Delete Data" class="btn btn-xs btn-icon btn-danger"><i class="icon-trash"></i></a>'
			);
	...
	public function insert()
	{
		...
		$item['admin_password'] = '$SMBB$'.substr(sha1(md5(md5($password))), 0, 50);
