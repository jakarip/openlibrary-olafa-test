		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="form-group">
                    ...
                    <div class="col-sm-9 prepend-icon">
                        <textarea class="form-control" name="inp[bk_reason]" id="bk_reason" required></textarea>
                    </div>
                </div>
			</div>
			<div class="modal-footer">
...
$(document).ready(function(){
	table = $('#table-member').DataTable({     
		...
		"order": [],  
		"ajax": {
			"url": "<?php echo site_url('index.php/historybooking/ajax_data')?>",
			"type": "POST"
		}, 
		"columnDefs": [
			{ 
...
function not_approved(id,data) { 
	...
	$('#modal_not_approved #bk_status').val(data); 
...
function save_not_approved(status) { 
	...
	if (form_not_approved.valid()) {
		$.ajax({
			url : 'index.php/historybooking/not_approved',
			type: "POST",
			data: form.serialize(),
			dataType: "JSON",
			beforeSend : function(){ 
...
function deletes(status) { 
	...
	$.ajax({
		url : 'index.php/historybooking/update',
		type: "POST",
		data: form.serialize(),
		dataType: "JSON",
		beforeSend : function(){ 
