                            ...
                            <div class="x_panel">
                                <div class="x_title">
									<table width="100%" border="0" cellpadding="2" cellspacing="2">  
										<tr >
											<td width="220px" >Jumlah Pengajuan</td>
											<td width="10px">:</td>
											<td><?php echo $total ?></td> 
											<td align="right"><a href="index.php/pengadaan/addpengajuan"><button type="button" class="btn btn-primary hijau">Tambah Pengajuan</button></a></td>
										</tr> 
									</table>									
                                        ...
                                        <tbody>
                                             <?php  
												...
												<tr class="<?php echo $style?>">  
													...
													<td class=""><?php echo  $row->pj_nik ?></td>
													<td class=""><?php echo  $row->pj_jabatan ?></td>  
													<td class=""><?php echo  $row->pj_kaprodi ?></td>
													<td class=""><?php echo  $row->pj_tanggal ?></td>  
													<td class=""><?php echo  $row->total ?></td>
													<td class="">
													<button type="button" onclick="Edit('<?php echo $row->pj_id ?>')" class="btn btn-xs green" title="Edit Data"><i class="fa fa-pencil-square-o" ></i></button>
													<button type="button" onclick="Delete('<?php echo $row->pj_id ?>')" class="btn btn-xs red" title="Delete Data"><i class="fa fa-trash-o"></i> </button>
													<a href="index.php/pengadaan/pengajuandetail/<?php echo $row->pj_id ?>"><button type="button" class="btn btn-xs blue" title="Detail"><i class="fa fa-file-o"></i></button></a></td>
												 </tr> 	
...
<div class="modal fade editPengajuan" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel">Edit Pengajuan</h4>
			</div>
			<div class="modal-body">
				<form id="editform" data-parsley-validate method="post" action="index.php/pengadaan/editPengajuanDb" class="form-horizontal form-label-left">
				<input type="hidden" id="id" name="id">
					...
					<div class="form-group">
					 ...
					 <div class="form-group">
						<label class="control-label col-md-2 col-sm-2 col-xs-12" for="first-name">Kaprodi <span class="required">*</span>
						</label>
						<div class="col-md-10 col-sm-10 col-xs-12">
...
function Delete(id)
{
	if(confirm(' ID = '+id+'\n Apakah anda yakin akan menghapus data tersebut ?')) {
		$.ajax({
			url:'index.php/pengadaan/delPengajuan',
			global:false,
			type:'post', 
			data: ({id : id}),
			async:false,
			success: function() { window.location='index.php/pengadaan/pengajuan' }
		});
	}
}
...
{ 
	$.ajax({
		url:'index.php/pengadaan/editPengajuan',
		global:false,
		type:'post',
		data: ({id : id}),
		dataType: "json",
		...
		success: function(result) {   
			...
			$('#editform #id').val(result.pj_id);
			$('#editform #nomor').val(result.pj_nomor);
			$('#editform #dosen').val(result.pj_dosen);
			$('#editform #nik').val(result.pj_nik);
			$('#editform #jabatan').val(result.pj_jabatan);
			$('#editform #kaprodi').val(result.pj_kaprodi);
		}
	});
}
