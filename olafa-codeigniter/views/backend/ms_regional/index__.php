                ...
                <div class="modal-body">
                    <div class="box-body" style="padding-bottom:0px">
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-sm-9">
                            	<?= form_dropdown('inp[reg_prov]', $prov, '', 'class="form-control input-sm select2" id="reg_prov" required') ?>                              
                                <input type="text" class="form-control input-sm" name="inp[reg_kec]" id="reg_kec" required>
                            </div>
                        </div>
                        <div class="form-group form-group-sm">
                            <label for="pus_name" class="col-sm-3 control-label">Kabupaten</label>
                            <div class="col-sm-9">
                            	<?= form_dropdown('inp[reg_kab]', array(), '', 'class="form-control input-sm  select2" id="reg_kab" required') ?>                               
                                <input type="text" class="form-control input-sm" name="inp[reg_kec]" id="reg_kec" required>
                            </div>
                        </div>
                        <div class="form-group form-group-sm">
    ...
    $(tb).dataTable({
		...
		/*'drawCallback': function( settings ) {
			...
			$.each(row, function(index, value) {
				$('#reg_prov').append('<option value="'+value[0]+'">'+value[0]+'</option>');
			});
			
			$.each(row, function(index, value) {
				$('#reg_prov').append('<option value="'+value[1]+'">'+value[1]+'</option>');
			});
		}*/
    });
