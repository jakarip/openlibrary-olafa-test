    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="frm" class="form-horizontal">
                <input type="hidden" name="inp[test_id_periode]" id="test_id_periode" value="<?=$id?>">
                <div class="modal-body">
                    <div class="box-body" style="padding-bottom:0px"> 
                        <div class="form-group form-group-sm">
                            <label for="pus_name" class="col-sm-3 control-label">Jadwal Tes</label>
                            <div class="col-sm-9">
                            	<input type="text" class="form-control input-sm" name="inp[test_date]" id="test_date" required>
                            </div>
                        </div> 
                    </div><!-- /.box-body -->
                </div>
                ...
                <button type="button" class="btn btn-success btn-labeled btn-xs" id="act-save" onclick="save('insert_test')">
                	<b><i class="icon-floppy-disk"></i></b> Simpan
                </button>
        		<button type="button" class="btn btn-success btn-labeled btn-xs" id="act-update" onclick="save('update_test')">
...
{  
	...
	$('#frmbox #test_date').daterangepicker({ 
...
function delete_test(id, txt)
{
	if(confirm('Data: '+txt+'\nApakah anda yakin akan menghapus data tersebut ?')) {
		$.ajax({
			url:baseurl+'/delete_test',
			global:false,
			async:true,
			type:'post',
			dataType:'json',
