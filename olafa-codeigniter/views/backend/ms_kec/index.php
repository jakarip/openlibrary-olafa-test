                ...
                <div class="modal-body">
                    <div class="box-body" style="padding-bottom:0px">
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-sm-9">
                            	<?= form_dropdown('prov', $prov, '', 'class="form-control input-sm select2" id="prov" required="true"') ?>
                            </div>
                        </div>
                        <div class="form-group form-group-sm">
                            <label for="pus_name" class="col-sm-3 control-label">Kabupaten</label>
                            <div class="col-sm-9">
                            	<select name="kab" id="kab" class="form-control input-sm select2" required="true"></select>
                            </div>
                        </div>
                        <div class="form-group form-group-sm">
                            <label for="pus_name" class="col-sm-3 control-label">Nama Kecamatan</label>
                            <div class="col-sm-9">
                            	<input type="text" class="form-control input-sm" name="inp[kec_name]" id="kec_name" required>
                            </div>
                        </div>
                    </div><!-- /.box-body -->
                </div>
...
var tb 		= '#table';      
var baseurl = '<?= y_url_admin() ?>/ms_kec';
...
{		
	$.ajax({
		...
		success: function(e) {
			...
			$('#prov').val(e.kec_prov_id).trigger('change');
			$('#kab').val(e.kec_kab_id).trigger('change');
			$('#kab_ori').val(e.kec_kab_id);
			$('#frmbox').modal({keyboard: false, backdrop: 'static'});
		},
		error : function() {
...
{		
	$.ajax({
		...
		success: function(e) {
			...
			$.each(e, function(key, value) {
				html += '<option value="'+value.kab_id+'">'+value.kab_name+'</option>';
			});
			$('#kab').html(html);
		},
		error : function() {
