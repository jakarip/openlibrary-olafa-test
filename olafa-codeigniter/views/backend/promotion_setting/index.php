<div class="panel panel-default flat">
    <table class="table table-bordered table-striped table-hover" id="table">
        <thead>
            <tr>
                <th width="50%">Registrasi Status</th>
                ...
                <div class="modal-body">
                    <div class="box-body" style="padding-bottom:0px">
                        <div class="form-group form-group-sm">
                            <label for="pus_name" class="col-sm-3 control-label">Registrasi Status</label>
                            <div class="col-sm-9">
                            	<?= form_dropdown('inp[set_status]', $promo, '', 'class="form-control input-sm" id="set_status" required') ?>
                            </div>
                        </div>
                        <div class="form-group form-group-sm">
                            <label for="pus_name" class="col-sm-3 control-label">Biaya</label>
                            <div class="col-sm-9">
                            	<input type="text" onkeyup="FormatCurrency(this)" class="form-control input-sm" name="inp[set_cost]" id="set_cost" required>
                            </div>
                        </div>
                    </div><!-- /.box-body -->
                </div>
...
var tb 		= '#table';      
var baseurl = '<?= y_url_admin() ?>/promotion_setting';
...
function active(sts, id, txt_id, txt)
{
	if(confirm('Data: '+txt+'\nApakah anda yakin akan '+(sts == 1 ? 'Aktifkan' : 'Non Aktifkan')+' data tersebut ?')) {
		$.ajax({
			...
			type:'post',
			dataType:'json',
			data: ({id : id, sts : sts, set_status : txt_id}),
			success: function(e) { 
				if(e.status == 'ok;') 
				{
					_reload();
