...
$(document).ready(function(){
	$('#member_number').tokenInput("index.php/sbkpprint/auto_data", {
		minChars: 5,
		tokenLimit: 1,
	...
	$(table).DataTable({     
		...
		"order": [],  
		"ajax": {
			"url": "<?php echo site_url('index.php/sbkpprint/ajax_json')?>",
			"type": "POST",
			"data" : function(data) {
    ...
    $("#download").click(function() {
		if($("#dates").val()!=""){
			...
			$.ajax({
				type : "POST",
				url: "index.php/sbkpprint/lists_excel",
				dataType:'JSON',
				data : {
					dates :  $("#dates").val(),
				},
				success: function(result){ 
					...
					hideLoading();
					window.location.href="index.php/sbkpprint/download/"+result;
				}  
			}); 
		}
...
function edit(id){
  ...
  reset();
	  ...
	  $.ajax({
		url : "<?php echo site_url('index.php/sbkpprint/edit')?>",
		type: "POST",
		data : {
			id : id
		},
...
function save(){
	var url;
	if(save_method == 'add'){
		url = "<?php echo site_url('index.php/sbkpprint/insert')?>";
	}else{
		url = "<?php echo site_url('index.php/sbkpprint/update')?>";
	}
