...
class CreamerModel extends CI_Model {
	...
	{  
		$catalog = $this->db->query("select codes from files")->result(); 
		return $catalog;
	} 
	
	function GetCatalogTA()
	{  
		$catalog = $this->db->query("select ki.code,tmf.NAMA_FAKULTAS fakultas,NAMA_PRODI prodi,author,editor,cc.code klasifikasi from knowledge_item ki 
left join classification_code cc on cc.id=ki.classification_code_id
 left join t_mst_prodi tmp on ki.course_code=tmp.C_KODE_PRODI 
left join t_mst_fakultas tmf on tmf.C_KODE_FAKULTAS=tmp.C_KODE_FAKULTAS 
where knowledge_type_id in (4,5,6)"); 
		return $catalog;
	} 
	
	function GetSubject()
	{  
		return $this->db->query("select * from subject_2016 group by prodi,kode_mk"); 
	} 
	function GetMappingMasterSubject($mk,$course)
	{  
		
		return $this->db->query("select kis.* from  knowledge_item_subject kis left join master_subject ms on ms.id=kis.master_subject_id 
				where curriculum_code='2014' and name='$mk' and course_code='$course'"); 
	}  

	function CheckMapping($kid,$msid)
	{  
		
		return $this->db->query("select * from  knowledge_item_subject where knowledge_item_id='$kid' and master_subject_id='$msid'"); 
	}  	
	function GetMasterSubject($mk,$course)
	{  
		return $this->db->query("select id from master_subject where curriculum_code='2016' and name='$mk' and course_code='$course'"); 
	}
	
	function GetNamaMK()
	{  
		return $this->db->query("select name, course_code from master_subject where name in (select name from master_subject where curriculum_code='2016')  and curriculum_code!='2016' "); 
	}   
	...
	{ 
		return $this->db->insert('master_subject', $data);
	}  
	
	function InsertMapping($data)
	{ 
		return $this->db->insert('knowledge_item_subject', $data);
	}  
	...
	{  
		return $this->db->query("select master_data_user user from member where id='$data'"); 
	}   
	 
	function getMasterData($username)
	{  
		return $this->db->query("select NO_HP from t_mst_mahasiswa where C_NPM='$username'"); 
	}   
	...
	{  
		return $this->db->query("select ks.code kode,title from knowledge_stock ks left join knowledge_item ki on ki.id=ks.knowledge_item_id where ks.id='$data'"); 
	}
	
	function updateMembers($user,$hp)
	{  
		return $this->db->query("update member set master_data_mobile_phone='$hp' where master_data_user='$user'"); 
	}
...
wd.member_id=m.id where abs(master_data_user) = 0 group by master_data_user order by master_data_user")->result(); 
 ...
 where id>'6165'  and workflow_id='1' group by created_by order by id")->result(); 
	...
	function getListWorkflowDoc()
	{ 	 
		return $this->db->query("select master_data_user,knowledge_item.title 
		from workflow_document 
		left join member on member.id=member_id
		join knowledge_item on workflow_document.title=knowledge_item.title
		where latest_state_id='5' and workflow_document.status='2'")->result(); 
	} 
}
?>