<?php
class Internationalfisikmodel extends CI_Model {
	...
	{ 	
		...
		return $this->db->query("SELECT *, count(ki.id) eks   
			...
			LEFT JOIN knowledge_stock ks ON ki.id = ks.knowledge_item_id where ki.knowledge_type_id='44' group by ki.id order by ki.id");
	}
	...
	function getbyquery($where="", $limit="")
	{
		...
		return $this->db->query("SELECT *, count(ki.id) eks  FROM knowledge_item ki LEFT JOIN knowledge_stock ks ON ki.id = ks.knowledge_item_id where ki.knowledge_type_id='44'   $where group by ki.id  ORDER BY publisher_name asc, published_year desc, title ASC");
	} 
