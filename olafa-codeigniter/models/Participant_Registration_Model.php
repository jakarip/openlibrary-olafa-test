...
class Participant_Registration_Model extends CI_Model 
{
	...
	**/
	...
	function getCountRegClosed($iduser)
	{
			return $this->db->query("SELECT COUNT(*) as jml 
																FROM participant_registration
								WHERE sreg_status_print = 'N' AND sreg_id_participant = '$iduser'");
	}
	...
	{
			return $this->db->query("SELECT *,qr.qr_id_user from ".$this->table." left join participant on par_id=sreg_id_participant
			left join pin on pin_id=sreg_id_pin
			LEFT JOIN questionnaire_response qr ON qr_id_user= par_id where pin_transaction_number='$id'");
	}
}
?>