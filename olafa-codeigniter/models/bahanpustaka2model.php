<?php
class BahanPustaka2Model extends CI_Model {
	...
	{ 
		return $this->db->query("
			...
			(select count(ks.id) from knowledge_item ki left join knowledge_stock ks on ks.knowledge_item_id=ki.id where ki.id=kis.knowledge_item_id group by ki.id) eks from knowledge_item_subject_copy kis 
	...
	{
		return $this->db->query("select ms.code kode_mk,ms.semester,ms.name nama_mk, SUBSTR(ms.code,-1) sks, (SELECT COUNT(*) FROM knowledge_item_subject_copy WHERE master_subject_id = ms.id) as jmljudul,ms.id id_kuliah FROM t_mst_prodi tp left join master_subject ms on tp.c_kode_prodi=ms.course_code WHERE tp.c_kode_prodi ='$jurusan' AND ms.curriculum_code = '$tahun' ORDER BY semester asc");
	...
	function getbukuref($kode, $where, $limit)
	{
		...
		from knowledge_item_subject_copy kis 
		...
		where $where kis.master_subject_id='$kode' order by ki.title
		");
