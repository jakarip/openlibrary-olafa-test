...
class Dashboard_Model extends CI_Model 
{
	...
	function get_year_visitor($location)
	{  
		return $this->db->query("select count(*) total from member_attendance 
		where attended_at like '".date("Y")."%' $location
		"); 
	}  
	
	function get_month_visitor($location)
	{ 
		return $this->db->query("select count(*) total from member_attendance 
		where attended_at like '".date("Y-m")."%' $location
		"); 
	}  
	
	function get_day_visitor($location)
	{ 
		return $this->db->query("select count(*) total from member_attendance 
		where attended_at like '".date("Y-m-d")."%' $location
		"); 
	}  
	
	function get_day_visitor_checkout($location)
	{ 
		return $this->db->query("select count(*) total from member_attendance_checkout 
		where attended_at like '".date("Y-m-d")."%' $location
		"); 
	}  
}
