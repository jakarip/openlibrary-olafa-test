...
class Bookingdisplay extends CI_Controller {
    ...
    public function display(){  	  
		...
		$data['holiday'] 	= implode(",",$dts);  
       	...
       	$data['menu']		= 'booking_display';  
       $this->load->view('theme_display',$data);
    }
			...
			if (($count->total<'2' && $this->session->userdata('user_id')==2) or ($this->session->userdata('user_id')==1)) {
				...
				if (($count->total<'2' && $this->session->userdata('user_id')==2) or ($this->session->userdata('user_id')==1)) {
					if(!$this->bm->checkExistBooking($startdate,$enddate,$item['bk_room_id'])->row()){
						...
						if (($room->room_min <= count($data2) && $this->session->userdata('user_id')==2) or ($this->session->userdata('user_id')==1)) {
							...
							$this->bm->addNotificationMobile($itemnotif);
