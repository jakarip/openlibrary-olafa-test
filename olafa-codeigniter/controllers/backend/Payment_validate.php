...
class Payment_validate extends CI_Controller 
{
	function __construct()
	{
		...
		$this->load->model('Payment_Validate_Model', 'sreg', TRUE);
	...
	{
		$data['view'] 	= 'backend/payment_validate/index';
		$data['title']	= 'Data Validasi Pembayaran';		
	...
	public function json()
	{
		...
		$columns = array( 
			array( 'db' => 'pin_id_periode', 'dt' => 0),
			array( 'db' => 'pin_transaction_number', 'dt' => 1 ),
			array( 'db' => 'periode_code', 'dt' => 2),
			array( 'db' => 'par_fullname', 'dt' => 3 ),
			array( 'db' => 'sreg_pay_date', 'dt' =>4 ),
			array( 'db' => 'sreg_pay_file', 'dt' => 5 ),
			array( 'db' => 'pin_id', 'dt' => 6)
		);
		...
		if(empty($param['where']))
				$param['where'] = " WHERE sreg_active='N'";  
		else $param['where'] .= " AND sreg_active='N'";   
		...
		{   
			$payment = '';
			if($row->sreg_pay_file!=null){
				$payment = '<a target="_blank" href="'.$row->sreg_pay_file.'" title="Bukti Pembayaran" class="btn btn-xs btn-icon btn-success"><i class="icon-file-check"></i> Bukti Pembayaran</a>';
			} 

			$status = "";
			$validate = "";
			if($row->sreg_active=='N' and ($row->sreg_payment=='0' or $row->sreg_payment==null)){
				$status = '<span class="label label-default">Belum Upload Bukti Pembayaran</span>'; 
			} 
			else 	if($row->sreg_active=='N' and $row->sreg_payment=='1'){ 
				$status = '<span class="label label-primary">Sudah Upload Bukti Pembayaran</span>'; 
				$validate = '<button title="Approve '.$row->pin_transaction_number.'" class="btn btn-xs btn-icon btn-success" onClick="validate(\''.$row->sreg_id_pin.'\',\''.$row->sreg_id.'\',\'validasi\')"><i class="icon-checkmark4"></i></button> <button title="Reject '.$row->pin_transaction_number.'" class="btn btn-xs btn-icon btn-danger" onClick="validate(\''.$row->sreg_id_pin.'\',\''.$row->sreg_id.'\',\'reject\')"><i class="icon-cross2"></i></button>';
			}
			else 	if($row->sreg_payment=='2'){ 
				$status = '<span class="label label-success">Tervalidasi</span>'; 
			} 
			else 	if($row->sreg_payment=='3'){ 
				$status = '<span class="label label-danger">Tidak Tervalidasi</span>'; 
			} 
			...
			$rows = array ( 
				...
				$row->periode_name.' / '.$row->periode_track_type.'<br>('.y_convert_date($row->periode_start_date,'d/m/Y').' - '.y_convert_date($row->periode_end_date,'d/m/Y').')',
				$row->par_fullname, 
				y_convert_date($row->sreg_pay_date,'d/m/Y H:i:s').'<br>'.$row->sreg_pay_name.'<br>'.$row->sreg_pay_bank.' - '.$row->sreg_pay_norek, 
				$payment,
				$status
			);
	...
	{
		if(!$this->input->is_ajax_request()) return false;
		if(!$this->input->post('pin')) return false;

		$status 		= $this->input->post('status');
		$pin_id 		= $this->input->post('pin'); 
		$sreg_id 		= $this->input->post('sreg_id'); 

		$pindb 		 	= $this->sreg->getall_view($pin_id)->row(); 
	 
		$data['setting'] = y_load_setting();
		$data['pin']     = $pindb;
		if($status=='reject'){  
			$this->sreg->edit($sreg_id, array('sreg_payment' => '3'));
			$body = $this->load->view('backend/payment_validate/email_not_valid', $data, true); 
			echo y_send_email($pindb->par_email, 'Pembayaran '.$data['setting']['website_name'].' Tidak Berhasil', $body);
			echo json_encode(array('status' => 'ok;', 'text' => '')); 
		} 
		else {  

			if( $this->pin->edit($pindb->pin_id, array('pin_status' => 'Y')) and $this->sreg->edit($sreg_id, array('sreg_active' => 'Y','sreg_payment' => '2','sreg_active_date' => date('Y-m-d H:i:s')))){  
				if($pindb->sreg_referral!=0){
					$cost = $this->db->query("SELECT set_cost as cost FROM promotion_setting WHERE set_status = 'token' AND set_active = '1'")->row(); 
					$item = array(
							'rp_id_participant' => $pindb->sreg_id_participant,
							'rp_id_ref' => $pindb->sreg_referral,
							'rp_id_pin' => $pindb->pin_id,
							'rp_set_status' => 'token',
							'rp_set_cost' => $cost->cost,
							'rp_date_trans' => date('Y-m-d H:i:s'),
							'rp_status_payment' => 'belum'
					);

					$this->db->insert('referral_payment', $item);
				}  
				 
				$body = $this->load->view('backend/payment_validate/email_valid', $data, true); 
				echo y_send_email($pindb->par_email, 'Pembayaran '.$data['setting']['website_name'].' Berhasil', $body);
				echo json_encode(array('status' => 'ok;', 'text' => '')); 
			}
			else {
	...
	public function send_email_kelulusan()
	{
		...
		$comp = $this->Ms_Component_Model->getall()->result();
		$comps = array();
		foreach($comp as $row){
		...
		if(!empty($dbs))
		{
			foreach($dbs as $db){
				...
				if($student)
				{
					...
					echo y_send_email($student->par_email, 'BUKTI KELULUSAN', $body,$setting['file_payment_method']);  
