...
class Simulation_Utbk extends CI_Controller 
{
	...
	{
		$data['view'] 	= 'backend/simulation_utbk/index';
		$data['title']	= 'Simulasi Kelulusan UTBK';		
		$data['icon']	= 'icon-podium';
		
		$temp = $this->temp->count();
		if($temp > 0)
			redirect(y_url_admin().'/simulation_utbk/step2');
			
		$data['count'] = $this->sr->getsimulation_utbk()->num_rows();
	...
	{
		$data['view'] 	= 'backend/simulation_utbk/step2';
		$data['title']	= 'Simulasi Kelulusan UTBK';		
		$data['icon']	= 'icon-podium'; 
	...
	**/
	...
	{
		$course = $this->Ms_Course_Model->getby(array('course_status' => 1,'course_type'=>'utbk'))->result();
		$result = $this->sr->getsimulation_utbk()->result();		
		$identifier = md5(time());
		...
		$objPHPExcel->getProperties()->setTitle("Data Simulasi UTBK Calon Mahasiswa")
						 			 ->setDescription("Data Simulasi UTBK Calon Mahasiswa");
		 
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'DATA SIMULASI UTBK CALON MAHASISWA ('.date('m/d/Y').')');
		...
		// $objPHPExcel->getActiveSheet()->setCellValue('F17', 'RATA-RATA NILAI');
		// $objPHPExcel->getActiveSheet()->mergeCells('F17:F18');
		...
		{   
			...
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, '17', $c->course_name); 
		...
		$col++;
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, '17', 'Nilai Total'); 
		$objPHPExcel->getActiveSheet()->mergeCellsByColumnAndRow($col, '17', $col, '18');
		
		$col++;
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, '17', 'Nilai Rata-Rata'); 
		$objPHPExcel->getActiveSheet()->mergeCellsByColumnAndRow($col, '17', $col, '18');
		...
		{		
			...
			if(!empty($row->sreg_score_un)) 
				$nilai = json_decode($row->sreg_score_un, true); 
			else
				$nilai = array();
			...
			{ 
				...
				if(isset($nilai[$c->course_id])){
					if ($nilai[$c->course_id]!=""){ 
							$bobot = $nilai[$c->course_id];
							$total_mk++;
					}
					else {
						$nilai[$c->course_id]=0;
			...
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $i, '500');
			$col++;
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $i, $bobot_total);
			$col++;
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $i, $bobot_total/$total_mk);
			$col++;
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $i, ($bobot_total/$total_mk >= 500 ? '1' : '0') );
			$col++;
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $i, ($bobot_total/$total_mk >= 500 ? '1' : '0') );
			$col++;
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $i, '');
	...
	{
		if(!$this->input->post('identifier')) redirect(y_url_admin().'/simulation_utbk/step2'); 
		...
		if(!empty($dbs))
		{
			...
			$data['view'] 	= 'backend/simulation_utbk/step3';
			$data['title']	= 'Simulasi Kelulusan UTBK';		
			$data['icon']	= 'icon-podium';
			// print_r($pin);
		...
		else redirect(y_url_admin().'/simulation_utbk/step2');
	}
