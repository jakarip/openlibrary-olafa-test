...
class Lists2 extends CI_Controller 
{
	function __construct()
	{
		...
		$this->load->model('Document2_Model', 'dm', TRUE);
	...
	public function index()
	{
		if (!$this->session->userdata('user_login')) {
			...
			$data['view'] = 'document/lists2/index';
			$data['title']	= 'Document';		
			$data['icon']	= 'icon-user';  
	...
	{  
		  ...
		  $file_path = __dir__ . '/assets/images/backgrounds/images.png';
		...
		$filetodownload = "bgss2.png";
		$result = $s3Client->getObject(array(
			'Bucket' => $bucket,
			'Key'    => $filetodownload,
			'SaveAs' => __dir__ . '/assets/images/backgrounds/bgss5.png'
		));
 
		echo $result['Body']->getUri() . "\n";
		
		$resultbool = $s3Client->doesObjectExist ($bucket, $filetodownload );
		if ($resultbool) {
	...
	{   
        ...
        {  
			...
			if($temp){
				...
				return redirect('document/lists2/'.$data);  
			}
			else { 
				header("location: /");
				die();
	...
	public function json()
    {
        ...
        foreach($result as $row)
        {
			...
			if($row->final_state_id==$row->latest_state_id and $row->wd_status=='1' and $session['membertype']=='1'){
				...
				$transfer = ' <a href="index.php/document/lists2/transfer/'.$row->wd_id.'" title="Transfer" class="btn btn-xs btn-icon btn-success"><i class="icon-calendar3"></i></a>';
			}
            ...
            $rows = array (
                ...
                $label,
                '<a href="index.php/document/lists2/edit/'.$row->wd_id.'" target="blank" title="Lihat Detail / Melakukan Approval" class="btn btn-xs btn-icon btn-primary"><i class="icon-book"></i></a> '.$deletes.' '.$transfer
            );
	...
	{ 
		if (!$this->session->userdata('user_login')) {
			...
			$data['view'] = 'document/lists2/edit';
	...
	public function save()
    {
		...
		redirect('document/lists2/edit/'.$wd_id);
    } 
