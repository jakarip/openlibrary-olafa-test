...
class BahanPustaka extends CI_Controller {
	...
	public function ajax_index()
	{
		...
		$table 		= "select c_kode_prodi,nama_fakultas,nama_prodi,
					...
					from t_mst_fakultas tmf left join t_mst_prodi tmp on tmp.c_kode_fakultas=tmf.c_kode_fakultas where (nama_prodi not like '%Pindahan%' and nama_prodi not like '%International%' and nama_prodi not like '%Internasional%') and nama_fakultas!='' and tmf.c_kode_fakultas='$faculty'
					";
		...
		foreach ($list as $dt) {
			...
			if(!$this->session->userdata('login')) 
				$action = '<div><a class="btn btn-sm btn-danger btn-embossed" target="_blank" href="index.php/bahanpustaka/mk/'.strtolower($dt->c_kode_prodi).'/'.$year.'" title="'.getLang('subject_detail').'"><i class="fa fa-file"></i></a> View Mata Kuliah</div>';
			else  
				$action = '<div><a class="btn btn-sm btn-danger btn-embossed" target="_blank" href="index.php/bahanpustaka/mk/'.strtolower($dt->c_kode_prodi).'/'.$year.'" title="'.getLang('subject_detail').'"><i class="fa fa-file"></i> View Mata Kuliah</a></div><br><div><button class="btn btn-sm btn-success btn-embossed" onclick="excel('."'".$dt->c_kode_prodi."','".$year."','".$grow_year."','ebook'".')" title="'.getLang('download').'"><i class="fa fa-cloud-download"></i> Download E-book</button></div><br><div><button class="btn btn-sm btn-success btn-embossed" onclick="excel('."'".$dt->c_kode_prodi."','".$year."','".$grow_year."','book'".')" title="'.getLang('download').'"><i class="fa fa-cloud-download"></i> Download Buku Tercetak</button></div>';
			$no++;
			$row = array();
			$row[] = $no;
	...
	public function excel()
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValue('G4', 'TAHUN TERBIT');
		$objPHPExcel->getActiveSheet()->setCellValue('H4', 'ISBN');
		$objPHPExcel->getActiveSheet()->setCellValue('I4', 'JUMLAH TOTAL (Eksemplar)');
		$objPHPExcel->getActiveSheet()->setCellValue('J4', 'JUMLAH TERSEDIA (Eksemplar)');
		$objPHPExcel->getActiveSheet()->setCellValue('L4', 'ID ITEM');
		$objPHPExcel->getActiveSheet()->setCellValue('M4', 'ID MK');
		$i 		= 5;
		$keys 	= "";
		foreach ($mk as $mks){
			 $buku = $this->BahanPustakaModel->getbukuref($mks->id, '', '',$type); 
			 echo $buku->num_rows;
			 if ($buku->num_rows()!=0){
				 foreach ($buku->result() as $key=>$bk){
					...
					$objPHPExcel->getActiveSheet()->setCellValue('G'.$i, $bk->published_year);
					$objPHPExcel->getActiveSheet()->setCellValueExplicit('H'.$i, $bk->isbn,  PHPExcel_Cell_DataType::TYPE_STRING);
					$objPHPExcel->getActiveSheet()->setCellValue('I'.$i, $bk->eks);
					$objPHPExcel->getActiveSheet()->setCellValue('J'.$i, $bk->tersedia);
					$objPHPExcel->getActiveSheet()->setCellValue('L'.$i, $bk->kiid);
					$objPHPExcel->getActiveSheet()->setCellValue('M'.$i, $bk->master_subject_id);
					$i++;
				 }
			}
			else {
		...
		$filePath = 'downloads/'.str_replace(" ","_",$prodi->nama_fakultas).'-'.str_replace(" ","_",$prodi->nama_prodi).'_'.$type.'.xlsx';
	...
	{
		if (!$this->input->is_ajax_request()) exit('No direct script access allowed'); 
		echo json_encode($this->BahanPustakaModel->totalcollection($this->input->post('year'),$this->input->post('grow_year'),$this->input->post('faculty'))->row());
	}
