...
class Dikti extends CI_Controller {
    ...
    function __construct() {
        parent::__construct(); 
		$this->load->model('DiktiModel');  
		if(!$this->session->userdata('login')) redirect('');	
    }

   function index() { 
		$data['total'] 	= $this->DiktiModel->getall()->num_rows();  
		$data['menu'] 	= 'dikti/dikti'; 
	...
	public function ajax_index()
	{
		...
		$table 		= "SELECT ki.id ids, title,publisher_name,isbn,published_year, count(ki.id) eks FROM knowledge_item ki LEFT JOIN knowledge_stock ks ON ki.id = ks.knowledge_item_id where ki.knowledge_type_id='43' group by ki.id order by ki.id";
