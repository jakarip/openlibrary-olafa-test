...
class Booking2 extends CI_Controller {
    ...
    public function bo(){  	  
		...
		// NotificationMobile('tester','1','Permintaan peminjaman Ruang Diskusi 2 pada tanggal 18-04-2023 08:00 - 19:30 telah disetujui.','31384','ruangan','fEPc5CK-SVagDiE4mG4lxL:APA91bHY8we4ff-GDiMCsy49D1g_g5yiwWCJhUbaGjVoijwAV9bMBsyIOeBrpxAFmIV_pjNMBq5_t7t6WjT_N3yMXFWUBzaHbsiTEkhz3in8wV1hQKA-uEzMNhgmhVz_zc_UaAIOBUNN');
		
		NotificationMobileBroadcast('tester','1','Permintaan peminjaman Ruang Diskusi 2 pada tanggal 18-04-2023 08:00 - 19:30 telah disetujui.','','','fEPc5CK-SVagDiE4mG4lxL:APA91bHY8we4ff-GDiMCsy49D1g_g5yiwWCJhUbaGjVoijwAV9bMBsyIOeBrpxAFmIV_pjNMBq5_t7t6WjT_N3yMXFWUBzaHbsiTEkhz3in8wV1hQKA-uEzMNhgmhVz_zc_UaAIOBUNN');
	...
	public function detail() {
		...
		$row = $this->bm->getBookingDetail($id)->row();
			...
			$desc = '<table width="100%"><tr><td width="100px">'.getLang('name').'</td><td width="20px;">:</td><td><b>'.$row->master_data_fullname.'</b></td></tr><tr><td>&nbsp;</td></tr><tr><td>Status</td><td>:</td><td>'.$status.'</td></tr><tr><td>&nbsp;</td></tr><tr><td style="vertical-align:top">'.getLang('member_name').'</td><td style="vertical-align:top">:</td><td>'.$list.'</td></tr><tr><td>&nbsp;</td></tr><tr><td>'.getLang('purpose').'</td><td>:</td><td>'.$row->bk_purpose.'</td></tr></table>'; 
			...
			if (($count->total<'2' && $this->session->userdata('user_id')==2) or ($this->session->userdata('user_id')==1)) {
				...
				if (($count->total<'2' && $this->session->userdata('user_id')==2) or ($this->session->userdata('user_id')==1)) {
					if(!$this->bm->checkExistBooking($startdate,$enddate,$item['bk_room_id'])->row()){
						...
						if ($room->room_min <= (count($data2)+1)) {
							...
							$messages 		= "[OPENLIBRARY]\n Anda melakukan permintaan peminjaman ruangan '".$room->room_name."' tgl ".$date." ".$starthour." - ".$endhour.".\n Akan dikonfirmasi jika telah diproses.";
